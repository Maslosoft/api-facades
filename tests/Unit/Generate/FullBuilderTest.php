...
class FullBuilderTest extends Unit
{
	public function testFullBuilder(): void
	{
		...
		$file = 'UERGREFUQQ==';
		$uploaded = $client->invoices->upload->post(UploadFile::create(file: $file, name: 'uploaded.pdf'));
		...
		$this->assertStringContainsString('name="file"; filename="uploaded.pdf"', (string)$client->captured['body']);
		...
		$this->assertSame(UploadFile::class, $uploadMethod->getParameters()[0]->getType()?->getName());
	}
