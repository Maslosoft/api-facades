...
class Builder extends BaseBuilder implements BuilderInterface
{
	...
	): ?array
	{
		...
		return [
			...
			'location' => 'body',
			'required' => $required,
			'type' => $this->isUploadFileSchema($propertySchema)
				? $this->describeUploadFileInputType()
				: $this->describeInputSchema($propertySchema, $models, $usedModelNames),
		];
	}
	 ...
	 * @param array<string, mixed> $schema
	 * @param array<string, Model> $models
	 * @return array<int, array{name: string, required: bool, contentType: string, filename?: string}>
	 */
	private function describeMultipartFields(array $schema, array $models): array
	{
		...
		foreach ((array)($resolvedSchema['properties'] ?? []) as $name => $propertySchema)
		{
			...
			if ($this->isUploadFileSchema($propertySchema))
			{
				$field['filename'] = (string)$name;
			}
			$fields[] = $field;
		}
	...
	/**
	 * FastAPI emits UploadFile parts in OpenAPI 3.1 as multipart string fields
	 * with contentMediaType but without the older format=binary marker.
	 *
	 * @param array<string, mixed> $schema
	 */
	private function isUploadFileSchema(array $schema): bool
	{
		...
		$format = strtolower((string)($schema['format'] ?? ''));
		if ($format === 'binary' || $format === 'base64')
		{
			return true;
		}

		return trim((string)($schema['contentMediaType'] ?? '')) !== '';
	}

	/**
	 * @return array{typeHint: string, docType: string, kind: string}
	 */
	private function describeUploadFileInputType(): array
	{
		$fqcn = '\\Maslosoft\\ApiFacades\\Models\\Generic\\UploadFile';
	...
	): array
	{
		...
		if ($this->isUploadFileSchema($schema))
		{
			$typeHint = '\\Maslosoft\\ApiFacades\\Models\\Generic\\UploadFile';
			$docType = '\\Maslosoft\\ApiFacades\\Models\\Generic\\UploadFile';
		}
	...
	private function resolveModelPropertyType(array $schema): string
	{
		...
		if ($this->isUploadFileSchema($schema))
		{
			return '\\Maslosoft\\ApiFacades\\Models\\Generic\\UploadFile';
		}
		$type = $this->primarySchemaType($schema);
