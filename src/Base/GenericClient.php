...
abstract class GenericClient
{
	...
	private function prepareMultipartBody(mixed $body, array $request): array
	{
		...
		foreach ($fields as $field)
		{
			...
			$parts[] = $this->createMultipartPart(
				$name,
				$value,
				(string)($field['contentType'] ?? ''),
				isset($field['filename']) ? (string)$field['filename'] : ''
			);
		}
	...
	private function normalizeMultipartBodyData(mixed $body, array $fields): array
	{
		if ($body instanceof UploadFile)
		{
			...
			throw new BadParamsException('Multipart UploadFile payload can only be used with a single multipart field.');
		}
	 ...
	 */
	private function createMultipartPart(string $name, mixed $value, string $contentType, string $filename): array
	{
		$part = ['name' => $name];
		$headers = [];
		if ($value instanceof UploadFile)
		{
			$part['contents'] = $this->normalizeUploadFileContents($value->file);
			$part['filename'] = $value->filename;
			$headers = $this->normalizeHeaders($value->headers);
		}
		else
		{
			$part['contents'] = $this->normalizeMultipartValue($value);
			if ($filename !== '')
			{
	...
	private function normalizeUploadFileContents(mixed $value): mixed
	{
		...
		throw new BadParamsException('UploadFile contents must be a string, stream, resource, or stringable value.');
	}
